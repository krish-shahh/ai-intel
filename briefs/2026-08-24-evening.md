...
people: []
companies: []
topics: [model-releases, claude-code, opencode]
---

## The signal

1. An anonymous "Ox Alpha" stealth coding model, increasingly fingerprinted as an unreleased Zhipu GLM-5.3 build, is soaking up free developer traffic
   What happened: Ox Alpha, a free 1M-context coding model from an undisclosed operator, appeared on OpenRouter on Aug 20; independent tokenizer, API error-code, and video-encoder fingerprinting reported today ties it to Zhipu's unreleased GLM-5.3 with high confidence, though Zhipu has not claimed it.
   Why it matters: developers are already routing production [[claude-code]] and [[opencode]] traffic through a model whose operator and data-retention terms are unconfirmed, and if the fingerprinting holds, Zhipu is field-testing near-frontier coding quality under a stealth wrapper before its own public [[model-releases]].
   Confidence: Reported
   Sources: [Tech Times](https://www.techtimes.com/articles/325244/20260823/coding-model-ox-alpha-retains-every-prompt-you-cannot-name-company-holding-them.htm) (independent) · [WCCFTech](https://wccftech.com/a-mysterious-ai-lab-is-offering-100-trillion-free-tokens-day-for-its-ox-alpha-model-as-evidence-points-to-zhipus-unreleased-glm/) (independent) · [OpenRouter](https://openrouter.ai/stealth/ox-alpha) (primary)

## Other

- Anthropic's community plugin marketplace mirror ([[claude-code]] / Cowork plugins, nightly-synced, automated security scanning before listing) is on GitHub's daily trending chart tonight [GitHub](https://github.com/anthropics/claude-plugins-community) (primary)
- Note on this run: Hacker News (Algolia), Polymarket, and Reddit were all unreachable this session (egress-blocked or unindexed), and X/Twitter could only be checked via search snippets, not direct fetch — narrower sourcing than usual tonight.
